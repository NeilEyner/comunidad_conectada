                ...
                <div class="col-lg-9">
                    ...
                    <div id="list-carr" class="shop_cart_wrap">
                        ...
                        foreach ($carrito as $prod):
                            ...
                            ?>
                            <div id="prod-carr-<?= $prod['ID'] . '-' . $prod['ID_Producto'] . '-' . $prod['ID_Artesano'] ?>"
                                class="single_shop_cart d-flex align-items-center flex-wrap">
                                <div class="cart_img mb-4 mb-md-0">
                                    <img loading="lazy" src="<?= base_url($prod['Imagen_URL']) ?>" alt="product"
                                        style="width:150px">
                                </div>
                                <div class="cart_cont">
                                    ...
                                    <ul id="carr-edit" class="list-inline pb-3">
                                        <li class="list-inline-item text-right">
                                            <input type="hidden" name="product-quanity" id="product-quanity"
                                                max="<?= $prod['Stock'] ?>" value="<?= $prod['Cantidad'] ?>">
                                        </li>
                                        <li class="list-inline-item"><span class="btn btn-success"
                                                id="<?= 'carr-btn-menos-' . $prod['ID_Producto'] . '-' . $prod['ID_Artesano']; ?>"
                                                onclick="editCarr('<?= base_url() ?>',<?= $prod['ID'] . ',' . $prod['ID_Producto'] . ',' . $prod['ID_Artesano'] . ',' . $prod['Stock'] . ',' . $prod['Cantidad'] . ',' . $prod['Precio'] ?>,-1)">-</span>
                                        </li>
                                        <li class="list-inline-item"><span class="badge bg-secondary"
                                                id="<?= 'var-val-' . $prod['ID_Producto'] . '-' . $prod['ID_Artesano']; ?>"><?= $prod['Cantidad'] ?></span>
                                        </li>
                                        <li class="list-inline-item"><span class="btn btn-success"
                                                id="<?= 'carr-btn-mas-' . $prod['ID_Producto'] . '-' . $prod['ID_Artesano']; ?>"
                                                onclick="editCarr('<?= base_url() ?>',<?= $prod['ID'] . ',' . $prod['ID_Producto'] . ',' . $prod['ID_Artesano'] . ',' . $prod['Stock'] . ',' . $prod['Cantidad'] . ',' . $prod['Precio'] ?>,1)">+</span>
                                        </li>
                                    </ul>
                                </div>
                                <div class=" ms-auto">
                                    <p id="<?= 'pre-' . $prod['ID_Producto'] . '-' . $prod['ID_Artesano']; ?>">
                                        <?= $prod['Precio'] * $prod['Cantidad'] ?> Bs.</p>

                                </div>
                                <div class=" align-items-center ms-md-auto ms-auto">
                                    <ul class="list-inline pb-3">

                                        <li class="list-inline-item btn p-0 m-0 "
                                            onclick="eliminar('<?= base_url() ?>',<?= $prod['ID'] . ',' . $prod['ID_Producto'] . ',' . $prod['ID_Artesano'] ?>)">
                                            <span class=""><i class="bi bi-trash " data-bs-toggle="tooltip"
                                                    data-bs-placement="right" title="eliminar"></i></span></li>
                    ...
                    <div class="cart_summary">
                        ...
                        <div id="res-carr">
                            ...
                            foreach ($carrito as $prod):
                                ...
                                ?>
                                <div id="res-<?= $prod['ID'] . '-' . $prod['ID_Producto'] . '-' . $prod['ID_Artesano'] ?>"
                                    class="cartsum_text d-flex justify-content-between">
                                    <p><?= $prod['Nombre'] ?></p>
                                    <p id="<?= 'pre2-' . $prod['ID_Producto'] . '-' . $prod['ID_Artesano']; ?>">
                                        <?= $prod['Precio'] * $prod['Cantidad'] ?> Bs.</p>
                                </div>
                                <?php
                            endforeach;
