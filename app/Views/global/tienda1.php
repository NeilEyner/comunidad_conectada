...
<script src="./assets/js/scripts/producto.js"></script>
                ...
                foreach ($categorias as $categoria): ?>
                    <li class="p-0 nav-item" style="cursor: pointer" onclick="cambiarCat('<?= $categoria['ID'] ?>')">
                        <a class=" d-flex nav-link justify-content-between p-0">
                            ...
                            <i id='cat-<?= $categoria['ID'] ?>' name='cat' class="pull-right fa fa-fw fa-check-circle mt-1"
                                hidden></i>
                        </a>
                    </li>
                <?php endforeach; ?>
                    ...
                    endforeach; ?>" <?php echo $hidden ?> name="prod">
                        <div class="card mb-4 product-wap rounded">
                            <div class="card rounded">
                                <img class="card-img rounded img-fluid" src="<?= $producto['Imagen_URL'] ?>"
                                        ...
                                        <?php if (session()->get('isLoggedIn')) {
                                            if ($puntajeRow->Puntaje == null) { ?>
                                                <li><a class="btn btn-success text-white"
                                                        id="h-t-<?= $producto['ID_Producto'] ?>-<?= $producto['ID_Artesano'] ?>"
                                                        onclick="mostrarPunt('<?= base_url() ?>',<?= $producto['ID_Producto'] ?>,<?= $producto['ID_Artesano'] ?>)"><i
                                                            id="heart-<?= $producto['ID_Producto'] ?>-<?= $producto['ID_Artesano'] ?>"
                                            ...
                                            <?php } else {
                                                ...
                                                <li><a class="btn btn-success text-white"><i
                                                            id="heart-<?= $producto['ID_Producto'] ?>-<?= $producto['ID_Artesano'] ?>"
                                                            class="fa fa-heart "></i></a></li>
                                            <?php }
                                        } ?>
                                        <li><a class="btn btn-success text-white mt-2"
                                                href="<?= base_url() . 'producto' . '/' . $producto['ID_Artesano'] . '/' . $producto['ID_Producto']; ?>"><i
                                                    class="far fa-eye"></i></a></li>
                                        <!-- <li><a class="btn btn-success text-white mt-2"
                                                onclick="anadirProducto('<?= base_url() ?>',<?= $producto['ID_Artesano'] ?>,<?= $producto['ID_Producto'] ?>,1,<?= $producto['Precio'] ?>)"><i
                                                    class="fas fa-cart-plus"></i></a></li> -->
                                        <li><a class="btn btn-success text-white mt-2"
                                                onclick="anadeProducto('<?= base_url() ?>', <?= $producto['ID'] ?>,<?= $producto['ID_Artesano'] ?>,<?= $producto['ID_Producto'] ?>, 1, <?= $producto['Precio'] ?>)"><i
                                                    class="fas fa-cart-plus"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                            <div class="card-body" style="text-align: center;"><b></b>
                                <a href="<?= base_url() . 'producto' . '/' . $producto['ID_Artesano'] . '/' . $producto['ID_Producto']; ?>"
                                    class="h3 text-decoration-none"><b><?= $prod['Nombre'] ?> </b></a>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document" width="50" style="width:fit-content">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button id="btn-mod-cancelar" type="button" class="btn btn-secondary"
                    data-bs-dismiss="modal">Cancelar</button>
                <button type="button" id="accept" onclick="puntuar()" class="btn btn-primary"
